/**
 * Proofly — deployer wallet sync-state persistence.
 *
 * Adapted from the ShadowPass-V2 / Flash Loan reference deployment pattern.
 * No wallet-sdk imports — keeps this module unit-testable independently.
 *
 * Why: without persistence, every `npm run deploy:proofly` rebuilds each child
 * wallet from seed and re-syncs against the chain. On Preprod that takes hours.
 * With persistence, sync resumes from the saved checkpoint.
 *
 * The state files live under `.midnight-wallet-state/` (gitignored) and contain
 * serialized wallet state derived from the deployer seed. They MUST NEVER be
 * committed. Delete them to re-sync from scratch.
 */
